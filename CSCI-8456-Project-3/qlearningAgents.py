    ...
    def computeActionFromQValues(self, state):
        ...
        if len(legalActions) == 0:
            ...
            for action in legalActions:
                qValueGoal = self.getQValue(state, action)
                actionVals[action] = qValueGoal
                if qValueGoal > qValues:
                    qValues = qValueGoal
            return random.choice([k for k,v in actionVals.items() if v == qValues])
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for item in features:
            self.weights[item] += self.alpha * difference * features[item]
